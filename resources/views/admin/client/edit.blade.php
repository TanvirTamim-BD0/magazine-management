    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.client.update", [$clientData->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                    <label for="country">Country</label>
                        <select class="form-control select2 {{ $errors->has('country') ? 'is-invalid' : '' }}" name="country">
                            ...
                            <option value="{{$country->name}}" {{$country->name == $clientData->country ? 'selected' : ''}} >{{$country->name}}</option>
                            @endforeach
                        </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('website') ? 'is-invalid' : '' }}" type="text" name="website" id="website" value="{{$clientData->website}}">
            </div>

            <div class="form-group">
                <label for="others_phone">Others Phone</label>
                <input class="form-control {{ $errors->has('others_phone') ? 'is-invalid' : '' }}" type="text" name="others_phone" id="others_phone" value="{{$clientData->others_phone}}">
            </div>

            <div class="form-group">
                <label for="others_email">Others Email</label>
                <input class="form-control {{ $errors->has('others_email') ? 'is-invalid' : '' }}" type="text" name="others_email" id="others_email" value="{{$clientData->others_email}}">
            </div>
            ...
            <div class="form-group">
                    <label for="area_code">Area</label>
                        <select class="form-control select2 {{ $errors->has('area_code') ? 'is-invalid' : '' }}" name="area_code">
                            ...
                            <option value="{{$areaCode->id}}" {{$areaCode->id == $clientData->area_code ? 'selected' : ''}}  >{{$areaCode->name}}</option>
                            @endforeach
                        </select>
            </div>
